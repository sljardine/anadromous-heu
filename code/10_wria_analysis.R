...
here::i_am("code/10_wria_analysis.R")
...
for (i in 1 : length(huc8s)) {
  ...
  heu_out1 <- data.frame(
    habitat_tot = heu_out1$habitat_tot, 
    habitat_natural = heu_out1$habitat_natural, 
    habitat_low_temp = heu_out1$habitat_low_temp, 
    habitat_species = heu_out1$habitat_species
  ) %>% mutate(method = "SRpa") 
  
  heu_rel_perf1 <- heu_out1 %>% 
    mutate(
      huc8 = huc8s[i], 
  ...
  heu_out2 <- data.frame(
    habitat_tot = heu_out2$habitat_tot, 
    habitat_natural = heu_out2$habitat_natural, 
    habitat_low_temp = heu_out2$habitat_low_temp, 
    habitat_species = heu_out2$habitat_species
  ) %>% mutate(method = "SRpp") 
  
  heu_rel_perf2 <- heu_out2 %>% 
    mutate(
      huc8 = huc8s[i],
      hab_tot_rp = habitat_tot/opt_out$habitat_tot,
      hab_nat_rp = habitat_natural/opt_out$habitat_natural,
...
rp <- rp1 %>% rbind(., rp2) %>% select(huc8, hab_tot_rp, hab_nat_rp, hab_cool_rp, hab_spec_rp, method)
rp_long <- rp %>% 
...
ggsave(
  filename = here("output", "wria.pdf"),
  units = "in",
  width = 6,
  height = 5,
)

summary(rp %>% filter(method == "SRpa"))
summary(rp %>% filter(method == "SRpp"))

# huc-level performance check ----
rp_wide <- rp %>%
  pivot_wider(
    names_from = method,
    values_from = c(hab_tot_rp, hab_nat_rp, hab_cool_rp, hab_spec_rp),
    names_glue = "{.value}_{method}"
  )

comparison <- rp_wide %>%
  mutate(
    SRpp_gt_SRpa_tot  = hab_tot_rp_SRpp  > hab_tot_rp_SRpa,
    SRpp_gt_SRpa_nat  = hab_nat_rp_SRpp  > hab_nat_rp_SRpa,
    SRpp_gt_SRpa_cool = hab_cool_rp_SRpp > hab_cool_rp_SRpa,
    SRpp_gt_SRpa_spec = hab_spec_rp_SRpp > hab_spec_rp_SRpa
  )

huc8_where_SRpp_is_better <- comparison %>%
  filter(
    SRpp_gt_SRpa_tot |
      SRpp_gt_SRpa_nat |
      SRpp_gt_SRpa_cool |
