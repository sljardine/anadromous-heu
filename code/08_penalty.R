...
heu_front <- heu_front %>% mutate(penalty = "none") 
...
for (i in 1 : length(gammas)) {
  ...
  heu_front_up <- heu_front_up %>% mutate(penalty = "# up") 
  
  heu_fronts_up[[i]] <- heu_front_up
  
  ## down penalty ----
  heu_front_dn <- get_pi_front(
    points = points_pi_dn %>% 
      mutate(
        owner = 1,
        area = 1
  ...
  heu_front_dn <- heu_front_dn %>% mutate(penalty = "# down") 
  
  heu_fronts_dn[[i]] <- heu_front_dn
  
  ## up and down penalty ----
  heu_front_updn <- get_pi_front(
  ...
  heu_front_updn <- heu_front_updn %>% mutate(penalty = "# up & down") 
  
  heu_fronts_updn[[i]] <- heu_front_updn
...
save(
  heu_front, heu_front_up, heu_front_dn, heu_front_updn, 
  heu_fronts, heu_fronts_up, heu_fronts_dn, heu_fronts_updn,
  budget_grd,
...
ggplot() +
  ...
  geom_line(
    data = heu_front %>% mutate(penalty = fct_relevel(penalty, c("none", "# up", "# down", "# up & down"))), 
    aes(x = expenditures/max(expenditures), y = habitat_tot, color = penalty, linetype = penalty),
    linewidth = 2
  ) + 
  geom_line(
    data = heu_fronts_dn[[penalty_lev]], 
    aes(x = expenditures/max(heu_front$expenditures), y = habitat_tot, color = penalty, linetype = penalty),
    linewidth = 2
  ) +
  geom_line(
    data = heu_fronts_up[[penalty_lev]], 
    aes(x = expenditures/max(heu_front$expenditures), y = habitat_tot, color = penalty, linetype = penalty),
    linewidth = 2
  ) + 
  geom_line(
    data = heu_fronts_updn[[penalty_lev]], 
    aes(x = expenditures/max(heu_front$expenditures), y = habitat_tot, color = penalty, linetype = penalty),
    linewidth = 2
  ) + 
