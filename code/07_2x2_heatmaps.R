...
library(vtable)
here::i_am("code/07_2x2_heatmaps.R")
...
## total habitat ----
htd_per <- get_hm_dat(
  opt_fronts1, heu_fronts2, attribute = "habitat_tot", obj_dfn = "multi"
)
htd_per <- htd_per %>%
...
hsoid_per <- get_hm_dat(
  opt_fronts1, heu_fronts2, attribute = "habitat_species", obj_dfn = "multi"
)
hsoid_per <- hsoid_per %>%
  mutate(ex = ifelse(Y == 0.3 & X > 0.24 & X < 0.26, 1, 0)) %>%
  dplyr::arrange(ex)
...
## cool habitat ----
hltd_per <- get_hm_dat(
  opt_fronts1, heu_fronts2, attribute = "habitat_low_temp", obj_dfn = "multi"
)
hltd_per <- hltd_per %>%
  mutate(ex = ifelse(Y == 0.3 & X > 0.24 & X < 0.26, 1, 0)) %>%
  dplyr::arrange(ex)
...
## natural habitat ----
hnd_per <- get_hm_dat(
  opt_fronts1, heu_fronts2, attribute = "habitat_natural", obj_dfn = "multi"
)
hnd_per <- hnd_per %>%
  mutate(ex = ifelse(Y == 0.3 & X > 0.24 & X < 0.26, 1, 0)) %>%
  dplyr::arrange(ex)
